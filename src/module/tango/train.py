...
def main():
    ...
    if args.augment and args.per_device_train_batch_size < 2:
        raise ValueError(
            "Augmentation requires per device train batch size of at least 2."
        )
    ...
    with accelerator.main_process_first():
        train_dataset = Text2AudioDataset(
            raw_datasets["train"], prefix, text_column, audio_column, args.num_examples
        )
        ...
        test_dataset = Text2AudioDataset(
            raw_datasets["test"], prefix, text_column, audio_column, args.num_examples
        )
            ...
            with accelerator.accumulate(model):
                ...
                with torch.no_grad():
                    ...
                    if args.augment and len(text) > 1:
                        ...
                        mixed_mel, _, _, mixed_captions = (
                            torch_tools.augment_wav_to_fbank(
                                audios, text, len(audios), target_length, stft
                            )
                        )
