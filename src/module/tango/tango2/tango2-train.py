...
def parse_args():
    ...
    parser.add_argument(
        "--train_file",
        type=str,
        default="data/train_audiocaps.json",
        help="A csv or a json file containing the training data.",
    )
    parser.add_argument(
        "--validation_file",
        type=str,
        default="data/valid_audiocaps.json",
...
class DPOText2AudioDataset(Dataset):
    def __init__(
        ...
        self.mapper = {}
        for index, audio_w, audio_l, text in zip(
            self.indices, self.audios_w, self.audios_l, inputs
        ):
            self.mapper[index] = [audio_w, audio_l, text]

        if num_examples != -1:
            self.inputs, self.audios_w, self.audios_l = (
                self.inputs[:num_examples],
...
def main():
    ...
    if accelerator.is_main_process:
        ...
        for i in range(len(dataset["train"])):
            sf.write(
                f"{dataset_dir}/audio_alpaca/chosen_{i}.wav",
                dataset["train"][i]["chosen"]["array"],
                samplerate=16000,
            )
            sf.write(
                f"{dataset_dir}/audio_alpaca/reject_{i}.wav",
                dataset["train"][i]["rejected"]["array"],
                samplerate=16000,
            )
    ...
    raw_dataset_train = HuggingfaceDataset.from_pandas(
        pd.DataFrame(
            {
                "prompt": prompt_list,
    ...
    if args.hf_model:
        ...
        model.load_state_dict(
            torch.load(
                "{}/pytorch_model_main.bin".format(hf_model_path), map_location="cpu"
            )
        )
    ...
    with accelerator.main_process_first():
        train_dataset = DPOText2AudioDataset(
            raw_dataset_train, prefix, "prompt", "chosen", "rejected", args.num_examples
        )
        eval_dataset = Text2AudioDataset(
            raw_dataset_valid_test["validation"],
            prefix,
            text_column,
            audio_column,
            args.num_examples,
    ...
    if args.freeze_text_encoder:
        ...
        if args.unet_model_config:
            ...
            optimizer_parameters = (
                ...
                + list(model.group_in.parameters())
                + list(model.group_out.parameters())
            )
    ...
    ) + sum(p.numel() for p in ref_unet.parameters() if p.requires_grad)
    ...
    if args.max_train_steps is None:
        args.max_train_steps = (
            args.num_train_epochs + args.sft_first_epochs
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(model):
                ...
                with torch.no_grad():
                    ...
                    mel_w, _, waveform_w = torch_tools.wav_to_fbank(
                        audio_w, target_length, stft
                    )
                    mel_l, _, waveform_l = torch_tools.wav_to_fbank(
                        audio_l, target_length, stft
                    )
                    ...
                    latent_w = unwrapped_vae.get_first_stage_encoding(
                        unwrapped_vae.encode_first_stage(mel_w)
                    )
                    latent_l = unwrapped_vae.get_first_stage_encoding(
                        unwrapped_vae.encode_first_stage(mel_l)
                    )
                ...
                if sft_epochs > 0:
                    ...
                    latents = latent_w
                    loss = model(
                        latents, text, ref_unet, validation_mode=False, sft=True
                    )

                else:
                    latents = torch.cat((latent_w, latent_l), dim=0)
                    loss = model(
                        latents, text, ref_unet, validation_mode=False, sft=False
                    )
        ...
        for step, batch in enumerate(eval_dataloader):
            with accelerator.accumulate(model) and torch.no_grad():
                ...
                val_loss = accelerator.unwrap_model(model).diffusion_forward(
                    true_latent, text, validation_mode=True
                )
        ...
        if accelerator.is_main_process and args.checkpointing_steps == "best":
            ...
            if (
                (epoch + 1) % args.save_every == 0 and sft_epochs < 0
            ):  ## Only save state after doing sft_first
                accelerator.save_state(
                    "{}/{}".format(args.output_dir, "epoch_" + str(epoch + 1))
                )

        if (
            accelerator.is_main_process
            and args.checkpointing_steps == "epoch"
            and sft_epochs < 0
        ):  ## Only save state after doing sft_first
            accelerator.save_state(
                "{}/{}".format(args.output_dir, "epoch_" + str(epoch + 1))
